...
class EmailValidateView(View):
	def post(self, request):
		...
		if not validate_email(email):
			return JsonResponse({'email_error_message': 'Invalid Email'}, status=400)


		if User.objects.filter(email=email).exists():
			return JsonResponse({'email_error_message': 'Sorry Email has been used, chose another one'}, status=409)
	...
	def post(self, request):
		...
		if not User.objects.filter(username=username).exists():
			if not User.objects.filter(email=email).exists():
				...
				email_body = 'Hi '+ user.username+', please use the  link below to  verify your account\n'+activate_url
				...
				messages.success(request, 'Account created succesfully.\n Please ckeck your email to verify the account before login.')
				return render(request, 'register.html')
